...
function Purchase() {
  ...
  useEffect(() => {
    const allChecked = Object.values(agreementCheckboxes).every(
      (value) => value
    );
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!passwordRegex.test(orderPassword)) {
      alert(
        "주문 비밀번호는 6~10자의 영문자와 숫자를 적어도 하나 이상 포함해야 합니다."
      );
      return; // 함수 실행 중지
    }
    ...
    await fetch(
      `http://${import.meta.env.VITE_URL_API}/api/handle/m/purchase`,
      {
        method: "POST",
        headers: {
      ...
      if (res.status == 201) {
        var userStats = await res.json();
        var expires = localStorage.getItem("Expiration");
        if (expires != null) {
          ...
          // 안한놈
          document.cookie = `userStats=${JSON.stringify(userStats)};`;
        }
                  ...
                  <div>
                    ...
                    <div className={styles.label}>
                      ...
                      <input
                        ...
                        name="addressZonecode"
                        value={addressZonecode}
                        placeholder="우편번호"
                        readOnly
                        required
                      ></input>
                      <div
                        className={styles.input_address_button}
                ...
                <div className={styles.contentMargin}>
                  <div style={{ fontSize: isMobile ? "3vw" : "1vw" }}>
                    ...
                    <div className={styles.orderPay}>
                      <p>
                        상품금액
                        <span>{itemsData?.m_price.toLocaleString()} P</span>
                      </p>
                      <p>
                        배송비 <span style={{ color: "green" }}>0P</span>
                      </p>
                    </div>
                    <div className={styles.underline} />
                  ...
                  <div>
                    ...
                    >
                      <input
                        type="checkbox"
                        className={styles.input_check}
                        checked={agreementCheckboxes.one}
                    ...
                    >
                      <input
                        type="checkbox"
                        className={styles.input_check}
                        checked={agreementCheckboxes.two}
                        onChange={() => handleCheckboxChange("two")}
                      />{" "}
                      필수 <span> 개인정보 수집 및 이용동의</span>
                    </label>
                    <label
                      className={styles.agreement}
                      style={{ color: "blue" }}
                    >
                      <input
                        type="checkbox"
                        className={styles.input_check}
                        checked={agreementCheckboxes.three}
                        onChange={() => handleCheckboxChange("three")}
                      />{" "}
                      필수 <span> 개인정보 제3자 제공동의</span>
                    </label>
                    <label
                      className={styles.agreement}
                      style={{ color: "blue" }}
                    >
                      <input
                        type="checkbox"
                        className={styles.input_check}
                        checked={agreementCheckboxes.four}
                        onChange={() => handleCheckboxChange("four")}
                      />{" "}
                      필수 <span> 구매회원 약관 동의</span>
                    </label>
                    <label
                      className={styles.agreement}
                      style={{ color: "blue" }}
                    >
                      <input
                        type="checkbox"
                        className={styles.input_check}
                        checked={agreementCheckboxes.five}
                        onChange={() => handleCheckboxChange("five")}
                      />{" "}
                      필수 <span> 전자금융거래 약관 동의</span>
                    </label>
                  </div>
                </div>
              </div>
